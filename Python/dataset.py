...
def ozone_data():
    ...
    k = 14;
    return np.reshape(V[:,0:k], k*24, order='F')
...
def portfolio_data(filepath=None):
    #import pandas as pd
    """
    portfolio_data()       
    ...
    if filepath is None:
        ...
        prices = pd.read_csv(filepath, sep=",", header=None)
...
def lq_estimation_data():
    """
    lq_estimation_data()
    Returns a 2 x 100 matrix with the measurement data for the linear quadratic 
    state estimation example of section 17.3.1.
    """
    #import numpy as np
    # data for linear quadratic estimation example
    ymeas = np.array([[1.71E+01,
    -1.84E+01,
    6.10E+00,
    -7.98E+00,
    2.27E+00,
...
def orth_dist_reg_data():
    """
    orth_dist_reg_data()
    Returns a tuple `(xd, yd)` with the data for the orthogonal distance 
    regression example on page 400.  
    """
    #import numpy as np
    xd = np.array([[8.6386309e-01],
                   [9.0618764e-01],
                   [2.0014826e-01],
                   [8.5939524e-01],
                   [7.9595557e-01],
